...
class SetpointResponse:
    """Echo body returned by PUT /setpoint.

    The heater acks a write with a slimmed-down Status: just the requested
    mode/setpoint plus Daily_Energy. Last_Update is reflected from the
    request (often 0) and shouldn't be treated as a wall-clock confirmation.
    """
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SetpointResponse":
        try:
            operative_mode = OperationMode(data.get("Operative_Mode"))
        except ValueError:
            operative_mode = None
        ...
        last_update = parse_timestamp(last_update_raw) if last_update_raw else None
