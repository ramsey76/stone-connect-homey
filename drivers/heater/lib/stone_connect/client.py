...
class StoneConnectHeater:
    ...
    async def _request(
        ...
        try:
            ...
            # PATCHED: this heater's firmware (PCB 571519332 v2.0, FW 1.0.2)
            # rejects GET requests without a JSON body ("incorrect API request").
            # Always send a JSON body — `{}` if the caller didn't provide data —
            # so the heater sees Content-Type: application/json + Content-Length.
            kwargs: Dict[str, Any] = {
                "method": method,
                "url": url,
    ...
    async def set_temperature_and_mode(
        ...
        Returns:
            Parsed echo of the heater's PUT /setpoint response.
        ...
        try:
            ...
            data = await self._request("PUT", "setpoint", body)
    ...
    async def set_temperature(
        ...
        return await self.set_temperature_and_mode(temperature, mode)

    async def set_operation_mode(self, mode: OperationMode) -> SetpointResponse:
        """
    ...
    # Convenience methods
    async def set_comfort_mode(self) -> SetpointResponse:
        """Set heater to comfort mode using the predefined comfort temperature."""
        return await self.set_operation_mode(OperationMode.COMFORT)

    async def set_eco_mode(self) -> SetpointResponse:
        """Set heater to eco mode using the predefined eco temperature."""
        return await self.set_operation_mode(OperationMode.ECO)

    async def set_antifreeze_mode(self) -> SetpointResponse:
        """Set heater to antifreeze mode using the predefined antifreeze temperature."""
        return await self.set_operation_mode(OperationMode.ANTIFREEZE)

    async def set_manual_temperature(self, temperature: float) -> SetpointResponse:
        """Set heater to manual mode with a specific temperature."""
        return await self.set_temperature(temperature, OperationMode.MANUAL)

    async def set_power_mode(self, power_level: OperationMode) -> SetpointResponse:
        ...
        return await self.set_operation_mode(power_level)

    async def set_standby(self) -> SetpointResponse:
        """Set heater to standby mode (essentially off but still connected)."""
        return await self.set_operation_mode(OperationMode.STANDBY)
